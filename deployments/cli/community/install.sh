...
function initialize(){
    ...
    if [ $? -eq 0 ]; then
        echo "Zeshan supports ${CPU_ARCH}" >&2
        echo "available"
        return 0
    else
        echo "" >&2
...
function download() {
    ...
    echo ""
    echo "Most recent version of Zeshan is now available for you to use"
    echo ""
...
function startServices() {
    ...
    if [ -n "$migrator_container_id" ]; then
        ...
        if [ $migrator_exit_code -ne 0 ]; then
            echo "Zeshan Server failed to start ❌"
            # stopServices
            echo
    ...
    echo "   Zeshan Server started successfully ✅"
    echo ""
...
function upgrade() {
    ...
    echo "Upgrading Zeshan to the latest release..."
    echo ""
